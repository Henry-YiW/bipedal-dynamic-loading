...
class PointFootBallBalance:
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.num_balls = self.num_actors - 1
        ...
        #states related for ball 
        self.load_observations = torch.zeros(self.num_envs, self.num_obs_load, dtype=torch.float, device=self.device)
        self.load_state_buffer = torch.zeros(self.num_envs, 20, self.num_obs_load, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        #state history for ball 
        self.obs_history_buf = torch.zeros(self.num_envs, cfg.env.obs_history_length * self.num_obs, 
    ...
    def reset(self):
        ...
        #modified to match the return entry number with addtional two return entries for ball
    ...
    def step(self, actions):
        ...
        for _ in range(self.cfg.control.decimation):
            ...
            # Compute torques with delayed actions
    ...
    def post_physics_step(self):
        ...
        # prepare quantities
        #Since the second dimesion now changes to all actors, we need to change the indexing to get
        #the state of the robot, which is the first actor
        ...
        #The same as above, the first actor is the robot, so we need to index it as 0
        ...
        #Same as above, the first actor is the robot, so we need to index it as 0
    ...
    def compute_observations(self):
        """Computes observations including proprioceptive and privileged information.
        Includes:
        - base angular velocity
        - projected gravity
        - joint positions and velocities
        - actions
        - clock inputs (sin and cos)
        - gait information
        """
        obs_buf, critic_obs_buf = self.compute_group_observations()
        
        if self.add_noise:
            noise = torch.randn_like(obs_buf) * self.noise_scale_vec
            obs_buf += noise
            
        self.proprioceptive_obs_buf = obs_buf
        self.critic_obs_buf = critic_obs_buf
    ...
    def compute_privileged_observations(self):
        """ Computes privileged observations
            Includes base proprioceptive, potentially height measurements, load observations,
            and other privileged info like base linear velocity.
        """
        if self.num_privileged_obs is not None:
    ...
    def _reset_dofs(self, env_ids):
        ...
        # --- Correct indexing for set_dof_state_tensor_indexed --- 
        # Need indices for the ROBOT actors (actor 0) in the specified envs
        # Note: Dof state tensor is likely ordered env0_robot, env1_robot, ...
        # It does NOT contain states for the ball actor as it has no DOFs.
        # Therefore, the indices passed should just be env_ids.
        env_ids_int32 = env_ids.to(dtype=torch.int32)
        # --- End correction ---
    ...
    def _reset_root_states(self, env_ids):
        ...
        """
        if len(env_ids) == 0:
            return # Nothing to do if no envs need reset

        # --- Step 1: Set Initial States Explicitly ---
        # Assign the robot's initial state ONLY to actor 0
        print("self.base_init_state: ", self.base_init_state)
        self.root_states[env_ids, 0, :] = self.base_init_state[env_ids]
        # Assign the ball's initial state ONLY to actor 1
        # Ensure self.ball_init_state is defined and initialized correctly elsewhere!
        self.root_states[env_ids, 1, :] = self.ball_init_state[env_ids]

        # --- Step 2: Apply Environment Origins ---
        # Add the origin offset to BOTH actors' positions AFTER setting initial states
        self.root_states[env_ids, :, :3] += self.env_origins[env_ids]

        # --- Step 3: Randomize Robot State ---
        # Apply random xy offset for robot base position ONLY if using custom origins (terrain)
        if self.custom_origins:
            self.root_states[env_ids, 0, :2] += torch_rand_float(-1., 1., (len(env_ids), 2), device=self.device)
        # Apply random base velocities ONLY to the robot (actor 0)
        self.root_states[env_ids, 0, 7:13] = torch_rand_float(-0.5, 0.5, (len(env_ids), 6), device=self.device)

        # --- Step 4: Randomize Ball State (Relative to Robot) ---
        # Apply random initial position offset for the ball (actor 1) relative to the robot's starting position
        # Note: robot position (self.root_states[env_ids, 0, :]) already includes env_origin offset
        self.root_states[env_ids, 1, 0:2] = self.root_states[env_ids, 0, 0:2] + torch_rand_float(-0.15, 0.15, (len(env_ids), 2), device=self.device)
        # Set ball height relative to robot base height + radius + small offset
        # Ensure self.ball_radius is defined correctly elsewhere!
        self.root_states[env_ids, 1, 2] = self.root_states[env_ids, 0, 2] + self.ball_radius + 0.2 + torch.rand(len(env_ids), device=self.device) * 0.01
        # Apply random initial velocities ONLY to the ball (actor 1)
        self.root_states[env_ids, 1, 7:13] = torch_rand_float(-0.5, 0.5, (len(env_ids), 6), device=self.device)

        # --- Step 5: Update Simulation ---
        # Calculate the correct flat list of actor indices to update in the simulation
        env_ids_int32 = env_ids.to(dtype=torch.int32)
        self.all_actor_indices = torch.arange(self.num_actors * self.num_envs, dtype=torch.int32, device=self.device).view(self.num_envs, self.num_actors)
        actor_indices = self.all_actor_indices[env_ids].flatten()
        # Set the updated root states in the simulation for the specified actors
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        #robots that walked far progress to harder terrain
        distance = torch.norm(self.root_states[env_ids, 0, :2] - self.env_origins[env_ids, :2], dim=1)
        move_up = distance > self.terrain.env_length / 2
        #robots that walked not far progress to easier terrain  
    ...
    def _init_buffers(self):
        ...
        self.base_position = self.root_states[:, 0, 0:3]
        self.base_orientation = self.root_states[:, 0, 3:7]
        self.base_lin_vel = self.root_states[:, 0, 7:10]
        self.base_ang_vel = self.root_states[:, 0, 10:13]
        ...
        self.last_actions = torch.zeros(self.num_envs, self.num_actions, self.cfg.env.num_action_history, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        # New buffers from official implementation
        self.action_fifo = torch.zeros(self.num_envs, self.cfg.control.action_delay_steps + 1, self.num_actions, dtype=torch.float, device=self.device)
        self.action_delay_idx = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self.clock_inputs_sin = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.clock_inputs_cos = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.gaits = torch.zeros(self.num_envs, self.cfg.env.num_gaits, dtype=torch.float, device=self.device)
        self.gait_indices = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        ...
        self.critic_obs_buf = None  # Will be initialized in compute_observations
    ...
    def _create_envs(self):
        """ Creates environments:
             ...
             2. loads the ball asset,
             3. For each environment
                3.1 creates the environment,
                3.2 calls DOF and Rigid shape properties callbacks,
                3.3 create robot actor with these properties and add them to the env
                3.4 create ball actor(s) with properties and add them to the env
        ...
        # --- Initialize base_init_state using the loop method from legged_robot --- 
        _base_init_state_1d = to_torch(base_init_state_list, device=self.device, requires_grad=False)
        self.base_init_state = torch.zeros(self.num_envs, 13, device=self.device)
        for idx in range(self.num_envs):
            self.base_init_state[idx] = _base_init_state_1d
        # --- End initialization ---
        start_pose = gymapi.Transform()
        start_pose.p = gymapi.Vec3(*self.base_init_state[0, :3]) # Use the first row for the initial start pose
        ...
        # --- Add initialization for ball asset outside the loop for efficiency ---
        # (You might want to make these parameters configurable via self.cfg)
        # ball_radius = 0.1 # Store this as a class attribute
        self.ball_radius = 0.1 # Example radius, make configurable if needed
        ball_options = gymapi.AssetOptions()
        ball_options.density = 100
        ...
        for i in range(self.num_envs):
            ...
            # --- Correctly apply random offset ONLY to robot start pos ---
            # pos = self.env_origins[i].clone() # Old code, got origin for BOTH actors, shape (2, 3)
            # pos[:2] += torch_rand_float(-1., 1., (2, 1), device=self.device).squeeze(1) # Old code, failed due to shape mismatch (2,3) += (2,)
            # start_pose.p = gymapi.Vec3(*pos) # Old code, used ambiguous combined pos
            
            # Get the robot's origin for this env (actor 0)
            robot_pos = self.env_origins[i, 0, :].clone() # Shape (3,)
            # Apply random XY offset to the robot's position
            robot_pos[:2] += torch_rand_float(-1., 1., (2, 1), device=self.device).squeeze(1) # Shape (2,) + (2,) works
            # Set the robot's start pose using the modified position
            start_pose.p = gymapi.Vec3(*robot_pos)
            # --- End correction ---

            rigid_shape_props = self._process_rigid_shape_props(rigid_shape_props_asset, i)
            self.gym.set_asset_rigid_shape_properties(robot_asset, rigid_shape_props)
            # Use the corrected start_pose for the robot actor
            ...
            # --- Add code for the second actor (ball) ---
            # Set initial pose for the ball (near the robot, slightly above ground)
            start_pose_ball = gymapi.Transform()
            # Use the MODIFIED robot_pos as the base for the ball's initial offset
            ball_initial_pos = robot_pos.clone() # Start from the robot's *actual* starting position for this env
            ball_initial_pos[0] += random.uniform(-0.1, 0.1) # Small random offset in x
            ball_initial_pos[1] += random.uniform(-0.1, 0.1) # Small random offset in y
            # ball_initial_pos[2] += ball_radius + 0.1 # Place ball slightly above ground relative to robot origin z
            ball_initial_pos[2] += self.ball_radius + 0.1 # Use self.ball_radius
            start_pose_ball.p = gymapi.Vec3(*ball_initial_pos)

            # Create the ball actor
            ball_name = "ball"
            ball_handle = self.gym.create_actor(
                env_handle, ball_asset, start_pose_ball, ball_name, i, 0, 0) # collision group 0, filter 0

            # Store ball handle
            self.load_handles.append(ball_handle)

            # Optionally: Set properties for the ball actor (color, physics) if needed
            # color = gymapi.Vec3(0.8, 0.2, 0.2) # Example: Red color
            # self.gym.set_rigid_body_color(env_handle, ball_handle, 0, gymapi.MESH_VISUAL, color)

            # Check actor count
            num_actors = self.gym.get_actor_count(env_handle)
            if num_actors != self.num_actors:
                 print(f"Warning: Environment {i} has {num_actors} actors, expected {self.num_actors}")
            # assert num_actors == self.num_actors, f"number of actors in the environment {i} is {num_actors}, expected {self.num_actors}"
            # --- End of added code for the second actor ---
    ...
    def _get_env_origins(self):
        ...
        if self.cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            # --- Ensure env_origins is (num_envs, num_actors, 3) even for plane ---
            # self.env_origins = torch.zeros(self.num_envs, 3, device=self.device, requires_grad=False)
            self.env_origins = torch.zeros(self.num_envs, self.num_actors, 3, device=self.device, requires_grad=False)
            # --- End change ---
            ...
            # --- Fill only the robot's origin (actor 0) ---
    ...
    # --- Add missing reward function ---
    def _reward_ball_dist(self):
        """ Penalize the ball's distance from the robot (adapted from legged_load_adapt) """
        tot_ball_dist_rew = 0
        robot_pos = self.root_states[:, 0, 0:3]
        if self.multi_balls:
            # Note: self.ball_positions is not defined in this class, use root_states directly
            for i in range(self.num_balls):
                ball_pos = self.root_states[:, i+1, 0:3] # Get pos for ball i+1
                ball_dist = torch.norm(ball_pos - robot_pos, dim=1)
                tot_ball_dist_rew += 1.0/(1.0 + ball_dist)
            return tot_ball_dist_rew
        else:
            # Assumes single ball is the second actor (index 1)
            ball_pos = self.root_states[:, 1, 0:3]
            ball_dist = torch.norm(ball_pos - robot_pos, dim=1)
            tot_ball_dist_rew += 1.0/(1.0 + ball_dist)
            return tot_ball_dist_rew
    
    # --- Add reward functions from legged_load_adapt (Attempt 2) ---
    def _reward_ball_lin_vel(self):
        """ Penalize the ball's linear velocity (adapted from legged_load_adapt) """
        tot_ball_vel_rew = 0
        if self.multi_balls:
            # Note: self.ball_linvels is not defined in this class, use root_states directly
            for i in range(self.num_balls):
                # Relative velocity of ball i+1 w.r.t robot base, expressed in base frame
                ball_vel = quat_rotate_inverse(self.base_quat, 
                                            (self.root_states[:, i+1, 7:10] - self.root_states[:, 0, 7:10]))
                tot_ball_vel_rew += 1.0 / (1.0 + torch.norm(ball_vel, dim=1))
            return tot_ball_vel_rew
        else:
            # Relative velocity of single ball (actor 1) w.r.t robot base (actor 0), expressed in base frame
            ball_vel = quat_rotate_inverse(self.base_quat, 
                                        (self.root_states[:, 1, 7:10] - self.root_states[:, 0, 7:10]))
            tot_ball_vel_rew += 1.0 / (1.0 + torch.norm(ball_vel, dim=1))
            return tot_ball_vel_rew

    def _reward_action_smoothness(self):
        """ Penalize non-smooth action sequences (adapted from legged_load_adapt) """
        # Penalize changes in actions using 2nd derivative
        term = torch.square(self.actions - 2.0 * self.last_actions + self.last_last_actions)
        return torch.sum(term, dim=1)

    def _reward_power(self):
        """ Penalize power consumption (adapted from legged_load_adapt) """
        # Penalize power = torques * velocities
        return torch.sum(torch.square(self.torques * self.dof_vel), dim=1)
    # --- End added reward functions ---
